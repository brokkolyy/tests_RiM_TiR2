# Page snapshot

```yaml
- generic [ref=e1]:
  - generic [ref=e2]:
    - banner [ref=e3]:
      - navigation [ref=e4]:
        - link "Конфигурация" [ref=e5] [cursor=pointer]:
          - /url: /configuration
          - button "Конфигурация" [ref=e6]
        - link "Мониторинг" [ref=e7] [cursor=pointer]:
          - /url: /monitoring
          - button "Мониторинг" [ref=e8]
        - link "Логирование" [ref=e9] [cursor=pointer]:
          - /url: /log
          - button "Логирование" [ref=e10]
        - link "Журналирование" [ref=e11] [cursor=pointer]:
          - /url: /journal
          - button "Журналирование" [ref=e12]
        - link "Графики" [ref=e13] [cursor=pointer]:
          - /url: /graph
          - button "Графики" [ref=e14]
        - link "Редактор HMI" [ref=e15] [cursor=pointer]:
          - /url: /HMIEditor
          - button "Редактор HMI" [ref=e16]
      - button "Settings" [ref=e17] [cursor=pointer]:
        - img [ref=e18]
      - button "Logout" [ref=e21] [cursor=pointer]:
        - img [ref=e22]
      - button "Toggle color mode" [ref=e25] [cursor=pointer]:
        - img [ref=e26]
    - generic [ref=e32]:
      - generic [ref=e34]:
        - button [ref=e35] [cursor=pointer]:
          - img [ref=e36]
        - button "Конфигурация" [ref=e39] [cursor=pointer]
        - button "Роутер" [ref=e40] [cursor=pointer]
        - generic [ref=e41]:
          - paragraph [ref=e42]: Конфигурация без названия
          - button "info" [ref=e43] [cursor=pointer]:
            - img [ref=e44]
        - img [ref=e46]
      - generic [ref=e55]:
        - generic [ref=e57]:
          - generic [ref=e59]:
            - generic [ref=e61]:
              - img [ref=e62]
              - paragraph [ref=e64]: Открыть контекстное меню
              - generic [ref=e65]: ПКМ
            - tree [ref=e67]:
              - treeitem "Прием" [expanded] [level=1] [ref=e71]:
                - generic [ref=e74]:
                  - button [ref=e75] [cursor=pointer]:
                    - img [ref=e76]
                  - paragraph [ref=e80]: Прием
          - separator [ref=e81]
          - tree [ref=e85]:
            - generic [ref=e88]:
              - treeitem "Передача" [expanded] [level=1] [ref=e89]:
                - generic [ref=e92]:
                  - button [ref=e93] [cursor=pointer]:
                    - img [ref=e94]
                  - generic [ref=e97]:
                    - paragraph [ref=e98]: Передача
                    - generic [ref=e99]:
                      - button [ref=e100] [cursor=pointer]:
                        - img [ref=e101]
                      - button [ref=e102] [cursor=pointer]:
                        - img [ref=e103]
                      - button [ref=e106] [cursor=pointer]:
                        - img [ref=e107]
              - treeitem "iec104 127.0.0.1 102 iec104_server" [expanded] [level=2] [ref=e110]:
                - generic [ref=e115]:
                  - button [ref=e116] [cursor=pointer]:
                    - img [ref=e117]
                  - generic [ref=e119]:
                    - generic "IEC-104 Сервер" [ref=e120]: iec104
                    - generic "IP-адрес" [ref=e121]: 127.0.0.1
                    - generic "Порт" [ref=e122]: "102"
                    - paragraph [ref=e123]: iec104_server
              - treeitem "asdu 1 asdu" [expanded] [level=3] [ref=e124]:
                - generic [ref=e130]:
                  - button [ref=e131] [cursor=pointer]:
                    - img [ref=e132]
                  - generic [ref=e134]:
                    - generic "ASDU" [ref=e135]: asdu
                    - generic "Адрес ASDU" [ref=e136]: "1"
                    - paragraph [ref=e137]: asdu
              - treeitem "ts1" [level=4] [ref=e138]:
                - generic [ref=e146]:
                  - img [ref=e147]
                  - img [ref=e150]
                  - generic "Тип сигнала" [ref=e155]: ts1
        - separator [ref=e156]
        - generic [ref=e161]:
          - generic [ref=e166]:
            - img [ref=e167]
            - paragraph [ref=e178]: Выберите узел в дереве приема или передачи
          - separator [ref=e179]:
            - generic:
              - img
          - generic [ref=e184]:
            - img [ref=e185]
            - paragraph [ref=e190]: Выберите узел в дереве переменных
        - separator [ref=e191]
        - generic [ref=e193]:
          - generic [ref=e195]:
            - img [ref=e196]
            - paragraph [ref=e198]: Открыть контекстное меню
            - generic [ref=e199]: ПКМ
          - tree [ref=e201]:
            - treeitem "Переменные" [expanded] [level=1] [ref=e205]:
              - generic [ref=e208]:
                - button [ref=e209] [cursor=pointer]:
                  - img [ref=e210]
                - paragraph [ref=e214]: Переменные
    - contentinfo [ref=e215]:
      - generic [ref=e216]:
        - generic [ref=e217]:
          - img [ref=e218]
          - paragraph [ref=e221]: "-------"
        - generic [ref=e222]:
          - img [ref=e223]
          - paragraph [ref=e226]: "----"
        - generic [ref=e227]:
          - img [ref=e228]
          - paragraph [ref=e230]: "----"
        - img [ref=e231]
        - 'generic "Текущая версия ПК: 1.99.999" [ref=e235]':
          - paragraph [ref=e236]: 1.99.999
  - region "bottom-end Notifications alt+T"
  - menu [active] [ref=e238]:
    - menuitem "Создать \"ASDU\"" [ref=e239]:
      - img [ref=e240]
      - text: Создать "ASDU"
    - separator [ref=e242]
    - menuitem "Переименовать Enter" [ref=e243]:
      - img [ref=e244]
      - text: Переименовать
      - generic [ref=e247]: Enter
    - menuitem "Удалить Backspace" [ref=e248]:
      - img [ref=e249]
      - text: Удалить
      - generic [ref=e252]: Backspace
    - menuitem "Игнорировать Ctrl+I" [ref=e253]:
      - img [ref=e254]
      - text: Игнорировать
      - generic [ref=e257]: Ctrl+I
    - separator [ref=e258]
    - menuitem "Вырезать Ctrl+X" [ref=e259]:
      - img [ref=e260]
      - text: Вырезать
      - generic [ref=e266]: Ctrl+X
    - menuitem "Копировать Ctrl+C" [ref=e267]:
      - img [ref=e268]
      - text: Копировать
      - generic [ref=e273]: Ctrl+C
    - menuitem "Вставить Ctrl+V" [disabled] [ref=e274]:
      - img [ref=e275]
      - text: Вставить
      - generic [ref=e279]: Ctrl+V
```