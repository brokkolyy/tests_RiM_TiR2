...
test.describe('Навигация', () => {
    ...
    test('Выбрать в списке - Однопозиционный ТС', async({page}) => {
        ...
        await expect(page.getByRole('group').filter({ hasText: 'Апертура' })).toBeHidden();
        await expect(page.locator('label').filter({ hasText: 'Команда' })).toBeHidden();//page.getByText('КомандаПрямоеВыбор/исполнитьПрямое')
    });
    ...
    test('Выбрать в списке - Двухпозиционный ТС', async({page}) => {
        ...
        await expect(page.locator('label').filter({ hasText: 'Апертура' })).toBeHidden();//page.getByRole('group').filter({ hasText: 'Апертура' }).toBeHidden();
        await expect(page.locator('label').filter({ hasText: 'Команда' })).toBeHidden();//page.getByText('КомандаПрямоеВыбор/исполнитьПрямое')
    });
    ...
    test('Выбрать в списке - ТИ масштабированное', async({page}) => {
        ...
        await expect(page.getByRole('group').filter({ hasText: 'Апертура' })).toBeVisible();
