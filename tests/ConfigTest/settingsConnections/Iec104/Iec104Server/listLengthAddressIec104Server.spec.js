...
test('Выбрать в списке - 1 байт', async ({ page }) => {
    ...
    await op1.click();
    await expect(page.getByRole('listbox', { name: 'Длина адреса объекта' })).toBeHidden();
...
test('Выбрать в списке - 2 байта', async ({ page }) => {
    ...
    await item.click();
    await expect(page.locator('[id="select::r1g::option:2"] > .chakra-select__itemIndicator > .css-s3mb0o')).toBeVisible();
});
test('Выбрать в списке - 3 байта', async ({ page }) => {
    const item = page.getByRole('combobox', { name: 'Длина адреса объекта' });
    await item.click();
    ...
    await item.click();
    await expect(page.locator('[id="select::r1g::option:3"] > .chakra-select__itemIndicator > .css-s3mb0o')).toBeVisible();
});
});