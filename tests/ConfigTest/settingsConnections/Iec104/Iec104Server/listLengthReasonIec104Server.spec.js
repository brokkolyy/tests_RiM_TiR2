...
test('Выбрать в списке - 1 байт', async ({ page }) => {
    const item = page.getByRole('combobox', { name: 'Длина причины передачи' });
    await item.click();

    const op1 = page.getByRole('option', { name: '1 байт' });
    await expect(op1).toBeVisible(); 
    await op1.click();
    await expect(page.getByRole('listbox', { name: 'Длина причины передачи' })).toBeHidden();
...
test('Выбрать в списке - 2 байта', async ({ page }) => {
    ...
    await item.click();
    await expect(page.locator('[id="select::r1e::option:2"] > .chakra-select__itemIndicator > .css-s3mb0o')).toBeVisible();
});
});