...
async function prepareField(page) {
    const field = page.getByRole('spinbutton', { name: 'Таймаут приема, сек' })
    await field.focus();
...
test.describe('Навигация', () => {
    ...
    test('Ввод минимально допустимого числа', async({page}) => {
        const field = await prepareField(page)
        const inputVal = '0';
        await field.fill(inputVal);
        await field.press('Enter');
    ...
    test('Проверить работоспособность кнопок', async({page}) => {
        const field = await prepareField(page)
